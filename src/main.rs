                ...
                match pnet_pkt {
                    Some(eth_pkt) => {
                        match eth_pkt.get_ethertype() {
                            ...
                            _ => {
                                warn!("Could not parse packet (EtherType: {}), trying Loopback", eth_pkt.get_ethertype());
                                let lo_pkt = Ipv4Packet::new(&cap_pkt.data[4..]);
                                match lo_pkt {
                                    Some(ipv4_pkt) => {
